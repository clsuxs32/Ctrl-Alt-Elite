...
(function(){
    ...
    function resize(){ 
        ...
        drops = Array.from({length: Math.floor(W/16)}, () => Math.random() * -50); 
    }
    ...
    function draw(){
        cx.fillStyle = 'rgba(2,10,2,0.045)'; 
        cx.fillRect(0,0,W,H);
...
function adminGuardarPost() {
    ...
    if(editIndex === -1) POSTS_DATA.push(postObj);
    else POSTS_DATA[editIndex] = postObj;
...
function validarCedula(){
    ...
    if(CEDULAS_AUTORIZADAS.includes(val)) {
        ...
        // Acceso Root para cédulas específicas
